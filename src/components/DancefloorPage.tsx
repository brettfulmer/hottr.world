...
const TOTAL_SCENES = 5 // 0-4 editorial, 5 = globe
...
// Staggered fade-in text line — cinematic movie intro feel
function FadeLine({ children, delay, active, className, style }: {
...
export default function DancefloorPage() {
  ...
  // Advance to next scene (used by both auto-timer and manual skip)
  const advance = useCallback(() => {
    setScene(prev => {
      if (prev > TOTAL_SCENES) return prev
      return prev + 1
    })
  }, [])

  // FIX #1: Robust auto-advance — single timer per scene, cleanly cleared
  useEffect(() => {
    if (scene > TOTAL_SCENES) return
    if (scene <= TOTAL_SCENES - 1) {
      // Editorial scenes auto-advance after their duration
      autoTimerRef.current = setTimeout(advance, SCENE_DURATIONS[scene])
    }
    return () => {
  ...
  // Manual skip (scroll/swipe/tap/key) — clears auto-timer to prevent double-fire
  ...
  // Interaction handlers for editorial scenes
  useEffect(() => {
    if (scene > TOTAL_SCENES) return
    const onWheel = (e: WheelEvent) => { e.preventDefault(); if (e.deltaY > 20) manualSkip() }
    const onKey = (e: KeyboardEvent) => {
      if (e.key === 'ArrowDown' || e.key === ' ' || e.key === 'Enter') manualSkip()
    }
    let touchY = 0
    const onTouchStart = (e: TouchEvent) => { touchY = e.touches[0].clientY }
    const onTouchEnd = (e: TouchEvent) => {
      if (e.changedTouches[0].clientY - touchY < -40) manualSkip()
    }
  ...
  // Globe: keyboard arrows = language
  useEffect(() => {
    if (scene <= TOTAL_SCENES) return
  ...
  useEffect(() => {
    ...
    const onEnd = (e: TouchEvent) => {
      ...
      if (Math.abs(dx) > 50) { dx < 0 ? cycleLang(1) : cycleLang(-1) }
    }
    ...
    <div className="w-full h-[100dvh] overflow-hidden bg-[#050608]">
      ...
      <div style={sceneStyle(0)}>
        <div className="px-8 sm:px-12 max-w-[820px]">
          <FadeLine active={scene === 0} delay={500}
            className="text-[22px] sm:text-[34px] md:text-[44px] font-bold text-white leading-[1.15] text-center"
            style={{ fontFamily: 'Poppins' }}>
            Music has always been the thing that brings strangers together.
          </FadeLine>
          <FadeLine active={scene === 0} delay={1500}
            className="text-[11px] sm:text-[14px] md:text-[16px] font-normal text-white/30 text-center mt-5 tracking-[0.18em] uppercase"
            style={{ fontFamily: 'Poppins' }}>
      ...
      <div style={sceneStyle(1)}>
        <div className="px-8 sm:px-12 max-w-[700px]">
          <FadeLine active={scene === 1} delay={400}
            className="text-[15px] sm:text-[19px] md:text-[23px] font-normal text-white/50 text-center leading-[1.85]"
            style={{ fontFamily: 'Poppins' }}>
      ...
      <div style={sceneStyle(2)}>
        <div className="px-8 sm:px-12 max-w-[700px]">
          <FadeLine active={scene === 2} delay={400}
            className="text-[15px] sm:text-[19px] md:text-[23px] font-normal text-white/40 text-center leading-[1.85]"
            style={{ fontFamily: 'Poppins' }}>
      ...
      <div style={sceneStyle(3)}>
        <FadeLine active={scene === 3} delay={300}
          className="text-[30px] sm:text-[48px] md:text-[68px] font-bold text-[#FF0CB6] text-center tracking-tight"
      ...
      <div style={sceneStyle(4)}>
        <div className="px-8 sm:px-12 max-w-[740px]">
          <FadeLine active={scene === 4} delay={400}
            className="text-[16px] sm:text-[20px] md:text-[24px] font-semibold text-white/60 text-center leading-[1.5] mb-6"
            style={{ fontFamily: 'Poppins' }}>
            In a world first for a commercial dance track.
          </FadeLine>
          <FadeLine active={scene === 4} delay={1200}
            className="text-[13px] sm:text-[16px] md:text-[18px] font-normal text-white/40 text-center leading-[1.9]"
            style={{ fontFamily: 'Poppins' }}>
            For the first time in history, a commercially released single is dropping in multiple languages — allowing over{' '}
            <span className="text-white font-semibold" style={{ whiteSpace: 'nowrap' }}>5&nbsp;billion</span>{' '}
      ...
      {/* ── SCENE 5+: THE GLOBE ── */}
      <div style={{ ...sceneStyle(TOTAL_SCENES + 1), justifyContent: 'flex-start', alignItems: 'center' }}>
        {gridOpen && (
        ...
        <div className="w-full h-full flex flex-col items-center relative" style={{ paddingTop: 'max(0.75rem, 1.5vh)' }}>
          ...
          {/* Background language name — FIX #5: matched Poppins font */}
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none overflow-hidden z-0">
            <span className="text-[15vw] font-extrabold leading-none tracking-tighter text-[#FF0CB6] opacity-[0.035] uppercase"
              style={{ fontFamily: 'Poppins' }}>
              {selected.name}
            </span>
          </div>
          ...
          {/* Language Card — FIX #5: Poppins throughout */}
              ...
              <div className="px-6 py-5 sm:px-7 sm:py-6">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <h3 className="text-[20px] sm:text-[26px] font-extrabold text-white tracking-tight uppercase leading-none" style={{ fontFamily: 'Poppins' }}>{selected.name}</h3>
                  </div>
